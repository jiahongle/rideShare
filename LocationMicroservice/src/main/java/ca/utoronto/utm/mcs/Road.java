...
public final class Road implements HttpHandler {

    private static Memory memory;
    private Database db;

    public Road(Memory mem, Database database) {
        memory = mem;
        db = database;
    }
    ...
    public void handlePut(HttpExchange r) throws IOException, JSONException {
        try {
            ...
            if (deserialized.has("RoadName") && deserialized.has("HasTraffic")) {
                roadName = deserialized.getString("RoadName");
                hasTraffic = deserialized.getBoolean("HasTraffic");
            ...
            int response = db.putRoad(roadName, hasTraffic);
            if (response == -1) {
